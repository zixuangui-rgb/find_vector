...
def main() -> None:
    ...
    np.savez_compressed(
        run_dir / "vectors" / "diffmean_vectors_by_layer.npz",
        **{key: np.stack(value) for key, value in vectors.items()},
    )
    write_csv(run_dir / "analysis" / "vector_geometry_by_layer.csv", rows)
    ...
    summary = {"best_emotion_layer": best_emotion, "best_belief_layer": best_belief, "layer_count_including_embedding": layer_count}
    write_json(run_dir / "analysis" / "vector_build_summary.json", summary)
